...
let doubling_resizer_with_shrinking curr_size _ new_length =
  assert(new_length >= 0);
  let rec grow size =
...
let doubling_resizer_without_shrinking curr_size _ new_length =
  assert(new_length >= 0);
  let rec grow size =
    if size >= new_length then size
    else if size < 0 then new_length
...
let compute_size a new_length =
  let new_size =
    a.resizer a.size a.length new_length
  in

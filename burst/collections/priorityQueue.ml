...
let remove_first_match h f =
  let find_index key i = function
    | Some idx -> Some idx
    | None -> if f key then Some i else None
  in
  match Hashtbl.fold find_index h.indices None with
  | Some idx -> remove_index h idx
  | None -> ()
...
sig
  ...
  val remove_first_match: t -> (elt -> bool) -> unit
  val clear: t -> unit
...
struct
  ...
  (* if not (Tbl.mem h.indices x) then failwith "PriorityQueue.set: index not registered" *)
  ...
  let remove_index h i =
    if is_empty h then failwith "PriorityQueue.remove_index: empty queue";
    (* Format.pp_print_string ppf "remove: "; *)
    let x = get h i in
    (* H.format ppf x;
    ...
    (* if Tbl.mem h.indices x then failwith "PriorityQueue.remove_index: not removed"; *)
    down_heap h i
  ...
  let remove_first_match h f =
    let find_index key i = function
      | Some idx -> Some idx
      | None -> if f key then
          (* Format.pp_print_string ppf "remove_first_match: ";
           H.format ppf key;
           Format.pp_print_newline ppf (); *)
           Some i
        else None
    in
    match Tbl.fold find_index h.indices None with
    | Some idx -> remove_index h idx
    | None -> ()
